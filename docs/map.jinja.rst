...
We will see a quick setup to configure the ``dero`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``dero`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``dero`` formulas under ``/srv/salt/dero/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/dero/parameters/dns:domain/
    ln -s dns:domain /srv/salt/dero/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/dero/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/dero/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``dero`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``dero`` formulas under ``/srv/salt/dero/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/dero/parameters/roles
...
We create a configuration for the role ``dero/server`` in ``/srv/salt/dero/parameters/roles/dero/server.yaml``:
...
We create another configuration for the role ``dero/client`` in ``/srv/salt/dero/parameters/roles/dero/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles dero/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles dero/client
...
.. code-block:: console
    ...
    mv /srv/salt/dero/parameters/dns:domain/  '/srv/salt/dero/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-dero-as-json:
      test.nop:
        - name: {{ dero | json }}
...
Here is an example based on `template-formula/dero/config/file.sls`_:
    ...
    dero-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='dero-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``dero`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ dero['winner'] }}
...
.. _template-formula/dero/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/dero/config/file.sls
